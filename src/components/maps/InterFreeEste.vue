<template>
	<div ref="maincontainer">
		<div class="videomap container left">
			<ul id="list" class="list" ref="list">
				<li @click="this.updateActiveIndex(0)" class="active">Item 1</li>
				<li @click="this.updateActiveIndex(1)">Item 2</li>
				<li @click="this.updateActiveIndex(2)">Item 3</li>
				<li @click="this.updateActiveIndex(3)">Item 4</li>
				<li @click="this.updateActiveIndex(4)">Item 5</li>
			</ul>
			<img src="../../assets/mappe/trieste-freeste-inland.png" alt="">
			<button class="prev-spot" @click="this.prevSpot"></button>
		</div>

		<div class="videomap container">
			<ul id="list" class="list2" ref="list2">
				<li @click="this.updateActiveIndex(5)">Item 6</li>
				<li @click="this.updateActiveIndex(6)">Item 7</li>
				<li @click="this.updateActiveIndex(7)">Item 8</li>
			</ul>
			<img src="../../assets/mappe/trieste-freeste-terminal.png" alt="">
			<button class="next-spot" @click="this.nextSpot"></button>
		</div>
		
	</div>
</template>

<script>
export default {
    name: 'InterFreeEste',
    props: {},
    data() {
        return {
			activeIndex: 0
		}
    },
    computed: {
		numSpots() {
			return this.$refs.maincontainer.querySelectorAll('#list li').length;
		}
	},
    methods: {
		updateActiveIndex(index) {
			const listItems = this.$refs.maincontainer.querySelectorAll('li');

			listItems.forEach(item => item.classList.remove('active'));
			listItems[index].classList.add('active');
			
			this.activeIndex = index;

			this.$emit('update-video-index', this.activeIndex)
		},
		nextSpot() {
			if(this.activeIndex<this.numSpots-1)
				this.activeIndex += 1
			else
				this.activeIndex = 0
				
			this.updateActiveIndex(this.activeIndex)
		},
		prevSpot() {
			if(this.activeIndex==0)
				this.activeIndex = this.numSpots-1
			else
				this.activeIndex -= 1

			this.updateActiveIndex(this.activeIndex)
		}
	},
    mounted() {}
}
</script>

<style scoped>
.left {
    transform: translateX(-19vw);
}

.list li:nth-child(1) {
	left: 66.54%;
	top: 63.63%;
}

.list li:nth-child(2) {
	left: 40.36%;
	top: 65.27%;
}

.list li:nth-child(3) {
	left: 66.54%;
	top: 46.36%;
}

.list li:nth-child(4) {
	left: 56.72%;
	top: 78.90%;
}

.list li:nth-child(5) {
	left: 68.90%;
	top: 35.63%
}

.list2 li:nth-child(1) {
	left: 55.45%;
	top: 40.72%;
}

.list2 li:nth-child(2) {
	left: 42%;
	top: 42.54%;
}

.list2 li:nth-child(3) {
	left: 16.90%;
	top: 41.45%;
}
</style>